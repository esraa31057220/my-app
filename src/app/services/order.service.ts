...
export class OrderService {
  ...
  checkout(shippingAddress: string): Observable<IOrder> {
    const params = new HttpParams().set('shippingAddress', shippingAddress);
    return this.http
      .post<unknown>(`${this.ordersUrl}/CheckOut`, null, { params })
      .pipe(map((raw) => this.normalizeOrder(raw)));
  }

  placeOrder(meta: PlaceOrderMeta): Observable<IOrder> {
    const shippingBlob = this.buildShippingBlob(meta);
    return this.checkout(shippingBlob).pipe(
  ...
  createOrder(order: IOrder): Observable<IOrder> {
    return this.checkout(order.shippingAddress ?? '');
  }
